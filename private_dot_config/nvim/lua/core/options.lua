...
opt.autoread = true              -- 外部変更時に自動リロード
...
-- 外部変更の自動検知（フォーカス復帰時・バッファ切替時・カーソル停止時）
...
vim.api.nvim_create_autocmd('VimEnter', {
  ...
  callback = function()
    ...
    if bufname ~= '' and vim.fn.isdirectory(vim.fn.fnamemodify(bufname, ':p:h')) == 1 then
      vim.cmd('lcd %:p:h')
      vim.cmd('pwd')
    end
  end
